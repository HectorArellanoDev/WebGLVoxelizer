const vsVoxelizerColor = `#version 300 es
...
void main() {
	...
	vec3 color1 = texelFetch(tColor, index2D(triangleIndex), 0).rgb;
	vec3 color2 = texelFetch(tColor, index2D(triangleIndex + 1.), 0).rgb;
	vec3 color3 = texelFetch(tColor, index2D(triangleIndex + 2.), 0).rgb;
	...
	colorData = vec4(0.);
	vec2 finalPos = vec2(0.);
	if(dist < 0.2) {
		finalPos = voxelPosition;
		colorData = vec4(averageColor, 1.);
	}
